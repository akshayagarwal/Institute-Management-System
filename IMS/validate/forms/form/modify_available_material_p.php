...
<title>Modify Material | Lead India Technologies</title>
...
<div class="content">
    <div id="signupbox">
       <div id="signuptab">
        <ul>
          <li id="signupcurrent"><a href=" ">Material Details</a></li>
        </ul>
      </div>
      <div id="signupwrap">
	<?php
	...
	$mcode=$_POST['mcode'];
	$matname=$mcode;
	$query="SELECT MCODE FROM materials WHERE MATNAME='$mcode'";
	$results=mysql_query($query);
	list($mcode)=mysql_fetch_row($results)
        ?>
      		<form  id="signupform" autocomplete="off" method="post" action="modify_available_material_pop.php">
	  		  <table>
	  		  <tr>
	  		  	<td class="label"><label for="material_name">Material Name</label></td>
	  		  	<td class="field"><?php echo $matname?></td>			
	  		  	<td class="status"></td>
	  		  </tr>
			  
	  		  <tr>
	  			<td class="label"><label for="quantity">Add Quantity</label></td>
	  			<td class="field"><input type="text" id="quantity" name="quantity2" maxlength="9"/></td>
				<td class="status"></td>
			  </tr>
