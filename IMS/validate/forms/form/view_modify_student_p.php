...
<title>View/Modify Student Details | Lead India Technologies</title>
...
<body>
	...
	$id=$_POST['id'];
	echo "Student ID : $id";
	$query="SELECT PID,COURSE,BATCH,FNAME,MNAME,LNAME,DOB,GENDER,SBI,BRANCH,ADDRESS1,ADDRESS2,ADDRESS3,PINCODE,MOBILE,LANDLINE,EMAIL,CHILD1,CHILD2,FEESPAID,REMARKS,TOTPOINTS FROM students WHERE ID='$id'";
	$results=mysql_query($query);
	list($pid,$course,$batch,$fname,$mname,$lname,$dob,$gender,$sbi,$branch,$address1,$address2,$address3,$pincode,$mobile,$landline,$email,$child1,$child2,$feespaid,$remarks,$totpoints)=mysql_fetch_row($results);
	$n=mysql_num_rows($results);
	if($n<=0)
	{
	echo "<h3> Invalid Student ID, Kindly Try Again </h3>";
	echo "<form action='view_modify_student_p.php' method='post'>";
	echo "<label for='id'>Student ID</label>";
	echo "<input type='text' name='id' label='id' maxlength='9'/>";
...
<div class="content">
    <div id="signupbox">
       <div id="signuptab">
        <ul>
          <li id="signupcurrent"><a href=" ">Student Details</a></li>
        </ul>
      </div>
      <div id="signupwrap">
      		<form id="signupform" autocomplete="off" method="post" action="view_modify_student_pop.php">
			<input type="hidden" name="id" value="<?php echo $id;?>"
	  		  <table>
	  		  <tr>
	  		  	<td class="label"><label for="id">Student ID</label></td>
	  		  	<td class="field"><?php echo $id;?></td>
	  		  	<td class="status"></td>
	  		  </tr>
			  <tr>
	  		  	<td class="label"><label for="pid">Referred By</label></td>
	  		  	<td class="field"><?php echo $pid;?></td>
	  		  	<td class="status"></td>
	  		  </tr>
	  		  <tr>
			  ...
			  <tr>
	  			<td class="label"><label for="course_name">Course</label></td>
	  			<td class="field"><?php echo $batch;?></td>
	  			<td class="status"></td>
	  		  </tr>
	  		  <tr>
			  ...
			  <tr>
	  			...
	  			<td class="field"><input id="sbi" type="text" name="sbi" value="<?php echo $sbi;?>" maxlength="11"/></td>
	  			<td class="status"></td>
	  		  </tr>
			  <tr>
	  			<td class="label"><label for="sbi">Bank Branch</label></td>
	  			<td class="field"><input id="sbi" type="text" name="branch" value="<?php echo $branch;?>" maxlength="50"/></td>
	  			<td class="status"></td>
	  		  </tr>
	  		  <tr>
